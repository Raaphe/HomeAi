...
export const StatisticScreen: FC<any> = observer(
  function StatisticScreen(_props) {
    ...
    const setChartDataSize = () => {
      ...
      const currentData = priceBySize.slice(
        currentIndexSize,
        currentIndexSize + 3
      )
      ...
      return {
        labels: currentData.map(
          (e: any) => `${formatNumber(e.min)} - ${formatNumber(e.max)} h`
        ),
        datasets: [
          {
    ...
    const setChartDataBed = () => {
      ...
      const currentData = avgPriceByBed.slice(
        currentIndexBed,
        currentIndexBed + 5
      )
      return {
    ...
    const setChartDataYear = () => {
      ...
      const currentData = salesByYear.slice(
        currentIndexYear,
        currentIndexYear + 5
      )
      return {
    ...
    const setChartDataState = () => {
      ...
      const currentData = avgPriceByState.slice(
        currentIndexState,
        currentIndexState + 5
      )
